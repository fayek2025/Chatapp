...
//              #0693e3 #37d67a #40bf75
                textFlow.setStyle("-fx-background-color: #EF4040; -fx-font-weight: bold; -fx-color: white; -fx-background-radius: 10px");
...
//        TextFlow textFlow = new TextFlow(imageView);
    ...
    public static void receiveMessage(String msg, VBox vBox) throws IOException {
        if (msg.matches(".*\\.(png|jpe?g|gif)$")){
            ...
            HBox hBoxName = new HBox();
            hBoxName.setAlignment(Pos.CENTER_LEFT);
            hBoxName.setPadding(new Insets(5 , 5 , 5 , 10));

            Text textName = new Text(name);
            TextFlow textFlowName = new TextFlow(textName);
            hBoxName.getChildren().add(textFlowName);
            textName.setStyle("-fx-text-fill : white ; -fx-font-weight: bold ; -fx-margin: 10px;  -fx-font-family : Big John;");


            textName.setFill(Color.color(1 , 1 , 1));
